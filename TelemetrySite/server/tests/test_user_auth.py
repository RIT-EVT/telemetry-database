...
def test_user_auth_login(client):
    login_payload = {"action": "login", "name": "test_user_valid", "password": "123"}
...
def test_user_auth_invalid_username(client):
    login_payload = {"action": "login", "name": "bad_username", "password": "123"}
    response = client.post("/Login", json=login_payload)
...
def test_user_auth_update_token(client):
    login_payload = {"action": "login", "name": "outdated_user", "password": "123"}
    response = client.post("/Login", json=login_payload)
    assert response.status_code == 200
    json_data = response.get_json()
...
def test_user_auth_signup(client):
    ...
    login_payload = {"action": "login", "name": "test_user_signup", "password": "123"}
...
def test_user_auth_invalid_challenge(client):
    ...
    response = client.post("/Login", json=invalid_signup)

    assert (
        response.status_code == 401
    ), "User created an acount without challenge number"

...
context_id = 1
config_file_path = "TelemetrySite/server/boardConfig.json"
    ...
    for data_package in tpdo_config:
        ...
        if table_to_insert != THROW_AWAY_CONFIG_STRING:
            ...
            if(data_package["signage"] == "signed"):
                insert_data_int = signed_bin_convert(int(insert_data_string, 2), data_size)
            else:
            ...
            cur.execute(SQL("INSERT into {} (" + column_string + ")  VALUES (" + insert_string + ")").format(*format_array), (*data_insert_array,))
    conn.commit()
    conn.close()
